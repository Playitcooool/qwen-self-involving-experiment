...
class LOPDTrainer:
    ...
    def cold_start(self) -> list[float]:
        ...
        for item in self.bank.items[: self.args.cold_start_steps]:
            ...
            student_logits, teacher_logits, _, target, _ = self._forward_pair(
    ...
    def update_rollout(self, task: Task, completion: str, reward: float) -> dict[str, Any]:
        ...
        student_logits, teacher_logits, reference_logits, target, _ = self._forward_pair(
            task, completion, latents
        )
        kl = reverse_kl_topk(student_logits, teacher_logits, self.args.top_k)
        kl_mean = kl.mean()
        reference_kl = reverse_kl_topk(
            student_logits, reference_logits.detach(), self.args.top_k
        )
        ...
        # Keep the privilege constraint composer-only.  Backpropagating this
        # term through the student would reward the student for lowering its
        # own probability, because the constraint is written as teacher minus
        ...
        loss = (
            distill_loss
            + self.args.reference_kl_weight * reference_kl_mean
            + self.beta * (self.args.margin - privilege)
            + self.args.anchor_weight * anchor_loss
        )
        ...
        if reward > 0.0:
            self.beta = max(
                0.0,
                self.beta
                + self.args.dual_step * (self.args.margin - float(privilege.detach())),
            )
        return {
            "task_id": task.task_id,
            "family": task.family,
            ...
            "reference_kl": float(reference_kl_mean.detach()),
